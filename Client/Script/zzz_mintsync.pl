...
sub hit_mintsync_service
{
	...
	print "\t\t".sha512_hex($password).":".$nonce if($debug);
...
elsif($verb =~ /retrieve/)
{
	...
	print "\tkeyslotkey_hex ".sha256_hex($passwordhash).$/ if($debug);
	...
	print "\trawKey_hex ".unpack("H*",$rawKey).$/ if($debug);
	
	print "\trawKey sha256_hex ".sha256_hex($rawKey).$/ if($debug);
